...
const logConsole = (msg, ...opt) => console.log(`\n${msg}`, ...opt, `\n\n`);

const dbData = JSON.parse(fs.readFileSync("./dbData.json"));
...
server.post("/create/:username", async (req, res) => {
  ...
  if (!pin || !username) throw { status: 400, message: "No username or pin!" };
  ...
  if (insertrows) {
    ...
    if (insertrows.affectedRows > 0) {
      const [newidrow] = await poolPromise.execute(Queries.getPlayerID, [
        username,
      ]);
...
server.post("/game/create", { logLevel: "warn" }, async (req, res) => {
  allowCredentials(res);
...
server.post("/game/:id/join", { logLevel: "warn" }, async (req, res) => {
  allowCredentials(res);
  const userid = parseInt(req.cookies.loggedas, 10);
...
server.post("/game/:id/leave", { logLevel: "warn" }, async (req, res) => {
  allowCredentials(res);
